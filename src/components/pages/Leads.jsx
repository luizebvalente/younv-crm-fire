...
import { Plus, Search, Filter, Edit, Trash2, Users, Calendar, DollarSign, TrendingUp, Check, RefreshCw, X, Tag, Settings, AlertTriangle, Clock, User } from 'lucide-react'
...
export default function Leads() {
  ...
  // NOVO: Estado para controle de usuário atual
  ...
  // NOVO: Função para obter o usuário atual do sistema de autenticação
  const getCurrentUser = async () => {
    try {
      // Esta função deve ser adaptada conforme seu sistema de autenticação
      // Exemplo usando Firebase Auth, Context API, ou outra solução
      const user = await firebaseDataService.getCurrentUser() // Você precisa implementar esta função
  ...
  // NOVO: Função para criar entrada de auditoria
  const createAuditEntry = (action, leadData, previousData = null) => {
    ...
    if (action === 'CREATE') {
      auditEntry.changes = {
        type: 'creation',
        data: {
          nome_paciente: leadData.nome_paciente,
          telefone: leadData.telefone,
          status: leadData.status,
          canal_contato: leadData.canal_contato
        }
      }
    } else if (action === 'UPDATE' && previousData) {
      const changes = {}
      
      // Comparar campos importantes e registrar mudanças
      const fieldsToTrack = [
        'nome_paciente', 'telefone', 'email', 'status', 'canal_contato',
        'medico_agendado_id', 'especialidade_id', 'valor_orcado',
        'orcamento_fechado', 'tags', 'observacao_geral'
      ]
      
      fieldsToTrack.forEach(field => {
        const oldValue = previousData[field]
        const newValue = leadData[field]
  ...
  // HANDLESUBMIT CORRIGIDO - INCLUI AUDITORIA
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // NOVO: Criar entrada de auditoria
      let auditEntry = null
      if (editingItem) {
        auditEntry = createAuditEntry('UPDATE', dataToSave, editingItem)
        
        // Adicionar auditoria ao lead existente
        const currentAudit = editingItem.audit_trail || []
        dataToSave.audit_trail = [...currentAudit, auditEntry]
        
        // Atualizar dados de modificação
        dataToSave.modified_at = new Date().toISOString()
        dataToSave.modified_by = {
          id: currentUser.id,
          nome: currentUser.nome,
          email: currentUser.email
        }
      } else {
        auditEntry = createAuditEntry('CREATE', dataToSave)
        
        // Adicionar auditoria inicial
        dataToSave.audit_trail = [auditEntry]
  ...
  // NOVA: Função para mostrar histórico de auditoria
  ...
  // NOVO: Componente para mostrar informações de auditoria
  const AuditInfo = ({ lead, compact = false }) => {
    if (!lead.created_by && !lead.modified_by) return null
        ...
        <div className="text-xs text-gray-500 space-y-1">
          {lead.created_by && (
            <div className="flex items-center gap-1">
              ...
              <span>Criado por {lead.created_by.nome}</span>
            </div>
          )}
          {lead.modified_by && (
            <div className="flex items-center gap-1">
              <Clock className="h-3 w-3" />
              <span>Editado por {lead.modified_by.nome}</span>
            </div>
          )}
        </div>
      )
      ...
      <div className="text-sm text-gray-600 space-y-2 p-3 bg-gray-50 rounded-lg">
        {lead.created_by && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-medium">{lead.created_by.nome}</div>
              <div className="text-xs text-gray-500">
                Criado em {new Date(lead.created_at).toLocaleString('pt-BR')}
              </div>
              <div className="text-xs text-gray-500">{lead.created_by.email}</div>
            </div>
          </div>
        )}
        ...
        {lead.modified_by && (
          <div className="flex items-center gap-2 pt-2 border-t">
            ...
            <div>
              <div className="font-medium">{lead.modified_by.nome}</div>
              <div className="text-xs text-gray-500">
                Última modificação em {new Date(lead.modified_at).toLocaleString('pt-BR')}
              </div>
              <div className="text-xs text-gray-500">{lead.modified_by.email}</div>
            </div>
          </div>
        )}
        
        {lead.audit_trail && lead.audit_trail.length > 1 && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => showLeadHistory(lead)}
            className="w-full mt-2"
          >
            <Clock className="h-4 w-4 mr-2" />
            Ver Histórico Completo ({lead.audit_trail.length} registros)
          </Button>
        )}
      </div>
    )
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              ...
              {currentUser && (
                <p className="text-sm text-gray-600">
                  Conectado como: <strong>{currentUser.nome}</strong> ({currentUser.email})
                </p>
              )}
            </div>
            <div className="flex gap-2">
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="!max-w-[70vw] max-h-[80vh] overflow-y-auto p-6 w-full">
                  ...
                  {/* NOVO: Mostrar informações de auditoria durante edição */}
                  {editingItem && (editingItem.created_by || editingItem.modified_by) && (
                    <Card className="mb-4">
                      <CardHeader>
                        <CardTitle className="text-sm">Informações de Auditoria</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <AuditInfo lead={editingItem} />
                      </CardContent>
                    </Card>
                  )}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left p-4">Auditoria</th>
                        ...
                        <tr key={lead.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-4">
                            <div className="max-w-[200px]">
                              <AuditInfo lead={lead} compact={true} />
                            </div>
                          </td>
      ...
      {/* NOVO: Dialog para Histórico de Auditoria */}
      <Dialog open={showAuditHistory} onOpenChange={setShowAuditHistory}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Clock className="h-5 w-5" />
              Histórico de Modificações - {selectedLeadHistory?.nome_paciente}
            </DialogTitle>
          </DialogHeader>
          
          {selectedLeadHistory?.audit_trail && (
            <div className="space-y-4">
              {selectedLeadHistory.audit_trail.map((entry, index) => (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="font-medium">{entry.user.nome}</span>
                          <span className="text-sm text-gray-500">({entry.user.email})</span>
                        ...
                        {entry.action === 'CREATE' && (
                          <div className="bg-green-50 border border-green-200 rounded p-3">
                            <h4 className="font-medium text-green-800 mb-2">Lead Criado</h4>
                            <div className="text-sm text-green-700 space-y-1">
                              <div><strong>Nome:</strong> {entry.changes.data.nome_paciente}</div>
                              <div><strong>Telefone:</strong> {entry.changes.data.telefone}</div>
                              <div><strong>Status:</strong> {entry.changes.data.status}</div>
                              <div><strong>Canal:</strong> {entry.changes.data.canal_contato}</div>
                            </div>
                          </div>
                        )}
                        ...
                        {entry.action === 'UPDATE' && Object.keys(entry.changes).length > 0 && (
                          <div className="bg-blue-50 border border-blue-200 rounded p-3">
                            <h4 className="font-medium text-blue-800 mb-2">Campos Alterados</h4>
                            <div className="space-y-2">
                              {Object.entries(entry.changes).map(([field, change]) => (
                                <div key={field} className="text-sm">
                                  <div className="font-medium text-blue-800">
                                    {field === 'nome_paciente' ? 'Nome do Paciente' :
                                     field === 'telefone' ? 'Telefone' :
                                     field === 'email' ? 'E-mail' :
                                     field === 'status' ? 'Status' :
                                     field === 'canal_contato' ? 'Canal de Contato' :
                                     field === 'valor_orcado' ? 'Valor Orçado' :
                                     field === 'orcamento_fechado' ? 'Status do Orçamento' :
                                     field === 'tags' ? 'Tags' :
                                     field === 'observacao_geral' ? 'Observações' :
                                     field}:
                                  </div>
                                  <div className="ml-4 text-blue-700">
                                    <div className="flex items-center gap-2">
                                      <span className="text-red-600">De: {
                                        field === 'tags' && Array.isArray(change.from) 
                                          ? change.from.map(tagId => getTagById(tagId)?.nome).filter(Boolean).join(', ') || 'Nenhuma tag'
                                          : String(change.from || 'Vazio')
                                      }</span>
                                      <span>→</span>
                                      <span className="text-green-600">Para: {
                                        field === 'tags' && Array.isArray(change.to)
                                          ? change.to.map(tagId => getTagById(tagId)?.nome).filter(Boolean).join(', ') || 'Nenhuma tag'
                                          : String(change.to || 'Vazio')
                                      }</span>
                                    </div>
                                  </div>
                                </div>
